...
class UnfurlTest(TestCase):
    ...
    def test_unfurl_discover_short_url_without_project_ids(self, mock_generate_chart):
        ...
        url = f"https://sentry.io/organizations/{self.organization.slug}/discover/results/?id={saved_query.id}&statsPeriod=24h&user={self.user.id}"
        link_type, args = match_link(url)
    ...
    def test_unfurl_discover_without_project_ids(self, mock_generate_chart):
        ...
        url = f"https://sentry.io/organizations/{self.organization.slug}/discover/results/?field=title&field=event.type&field=project&field=user.display&field=timestamp&name=All+Events&query=&sort=-timestamp&statsPeriod=24h&user={self.user.id}"
        link_type, args = match_link(url)
